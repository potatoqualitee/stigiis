function Get-StgAppPoolRapidFailInterval {
    ...
    .EXAMPLE
        PS C:\> Get-StgAppPoolRapidFailInterval -ComputerName web01

        Gets required information from web01

    .EXAMPLE
        PS C:\> Get-StgAppPoolRapidFailInterval -ComputerName web01 -Credential ad\webadmin
    ...
    begin {
        ...
        $scriptblock = {
            ...
            foreach ($pool in $pools) {
                $protectioninterval =  (Get-ItemProperty -Path "IIS:\AppPools\$($pool)" -Name $filterpath).Value
                if ([int]([timespan]$protectioninterval).TotalMinutes -le 5) {
                    $compliant = $true
                } else {
                    $compliant = $false
                }
                ...
                [pscustomobject] @{
                    ...
                    Value           = [int]([timespan]$protectioninterval).TotalMinutes
                    Compliant       = $compliant
