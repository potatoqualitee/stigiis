...
<#
    ...
    .EXAMPLE
        PS C:\> Set-StgCertificate -ComputerName web01

        Updates specific setting to be compliant on web01

    .EXAMPLE
        PS C:\> Set-StgCertificate -ComputerName web01 -Credential ad\webadmin
        ...
        foreach ($computer in $ComputerName) {
            try {
                Invoke-Command2 -ComputerName $computer -Credential $credential -ScriptBlock $scriptblock |
                    Select-DefaultView -Property Id, ComputerName, DNS, ExpirationDate, Version, HasPrivateKey |
